...
/**
 * Tarjeta compacta para móvil: imagen + nombre en zig-zag. Al pulsarla despliega
 * toda la información y, además, recoloca imagen y nombre con una animación
 * nativa tipo FLIP (sin librería):
 *  - la imagen pasa de su posición lateral pequeña al centro a todo el ancho,
 *  - el nombre pasa de estar al lado a quedar debajo de la imagen.
 * Se mide la posición/tamaño antes y después del cambio de layout, se invierte
 * con `transform` y se anima hasta la identidad → efecto translate real.
 */
export default function PacoCardMobile({
  paquito,
  reverse = false,
}: PacoCardMobileProps) {
  // Deep-link desde la home (`#<slug>`): leemos el hash como sistema
  // externo (sin setState en effect → sin warning; snapshot de servidor `false`
  // → sin mismatch de hidratación). El override del usuario permite cerrar la
  // card aunque el hash la siga apuntando.
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2
          ref={nameRef}
          className="font-chunko text-2xl uppercase leading-none"
          style={{ color: accent }}
        >
          {paquito.name}
        </h2>
